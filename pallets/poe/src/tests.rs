...
fn create_claim_failed_when_claim_exist() {
    new_test_ext().execute_with(|| {
        ...
        // 再次创建存证
        assert_noop!(PoeModule::create_claim(signer, claim.clone()),Error::<Test>::ClaimAlreadyExist);
        // 检查存证
        assert_eq!(Proofs::<Test>::get(&claim), Some((SIGNER_1, Pallet::<Test>::block_number())));
    });
}
...
fn transfer_claim_failed_when_claim_not_exist() {
    new_test_ext().execute_with(|| {
        ...
        // 转移存证
        assert_noop!(PoeModule::transfer_claim(signer, SIGNER_2, claim.clone()),Error::<Test>::ClaimNotExist);
        // 检查存证
        assert_eq!(Proofs::<Test>::get(&claim), None);
    })
}
...
fn transfer_claim_failed_when_not_owner() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(PoeModule::create_claim(signer, claim.clone()));
        // 转移存证
        assert_noop!(PoeModule::transfer_claim(signer2, SIGNER_3, claim.clone()),Error::<Test>::NotClaimOwner);
        // 检查存证
        assert_eq!(Proofs::<Test>::get(&claim), Some((SIGNER_1, Pallet::<Test>::block_number())));
    })
}
...
fn transfer_claim_failed_when_transfer_to_owner() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(PoeModule::create_claim(signer.clone(), claim.clone()));
        // 转移存证
        assert_noop!(PoeModule::transfer_claim(signer, SIGNER_1, claim.clone()),Error::<Test>::TransferToOwner);
        // 检查存证
        assert_eq!(Proofs::<Test>::get(&claim), Some((SIGNER_1, Pallet::<Test>::block_number())));
    })
}
